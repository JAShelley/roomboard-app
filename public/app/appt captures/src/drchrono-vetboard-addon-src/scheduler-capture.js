(function () {
  ...
  if (window.__vetBoardDrchronoCaptureLoaded) return;
  window.__vetBoardDrchronoCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-drchrono-overlay";
  const TOAST_ID = "vetboard-drchrono-toast";
  const BADGE_ID = "vetboard-drchrono-badge";
  const AUTH_PANEL_ID = "vetboard-drchrono-auth-panel";
  const MODAL_ID = "vetboard-drchrono-modal";
  const BACKDROP_ID = "vetboard-drchrono-backdrop";
  const HOVER_CLASS = "vetboard-drchrono-hover";
  ...
  // DrChrono uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)drchrono\.com$/i;
  ...
  // DrChrono card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // DrChrono appointment type → RoomBoard label mapping
  ...
  // DrChrono-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need DrChrono's own
    ...
    // Wait briefly so DrChrono's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the DrChrono appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (DrChrono's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by DrChrono) never embed time text in the card body — the
    ...
    // ── DrChrono card text pattern ────────────────────────────────────────────
    // DrChrono cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  function parseAppointment(card) {
    ...
    // Try DrChrono-specific detail panel first (the right-side info pane)
    ...
    // Try to parse DrChrono's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → DrChrono card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → DrChrono card parse → generic guess
    ...
    // Doctor: DrChrono calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse DrChrono card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip DrChrono patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the DrChrono right-side appointment detail panel
  // DrChrono labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate DrChrono's right-side detail panel (appears on click/hover)
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (DrChrono format)
  ...
  // ── Color label matching (DrChrono-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. DrChrono type map — highest priority
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard DrChrono capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the DrChrono appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-drchrono-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-drchrono-styles";
    style.textContent = `
      .${HOVER_CLASS} {
