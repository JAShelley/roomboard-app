(function () {
  ...
  if (window.__vetBoardSimplePracticeCaptureLoaded) return;
  window.__vetBoardSimplePracticeCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-simplepractice-overlay";
  const TOAST_ID = "vetboard-simplepractice-toast";
  const BADGE_ID = "vetboard-simplepractice-badge";
  const AUTH_PANEL_ID = "vetboard-simplepractice-auth-panel";
  const MODAL_ID = "vetboard-simplepractice-modal";
  const BACKDROP_ID = "vetboard-simplepractice-backdrop";
  const HOVER_CLASS = "vetboard-simplepractice-hover";
  ...
  // SimplePractice uses FullCalendar — target those event elements first, then fall back generically
  ...
  const DOCTOR_RE = /\b(?:dr\.?|doctor|lmft|lcsw|lpc|lpcc|lmhc|licsw|psyd|phd|md|pmhnp|therapist|counselor|clinician)\b/i;
  const DOCTOR_NAME_RE = /\b(?:dr\.?|doctor|lmft|lcsw|lpc|lpcc|lmhc|licsw|psy\.?d|ph\.?d|m\.?d\.?|pmhnp)\b/i;
  // Fast-path page detection for this platform's real scheduler domains.
  const HOST_HINT_RE = /(?:^|\.)simplepractice\.com$/i;
  ...
  // SimplePractice card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // SimplePractice appointment type → RoomBoard label mapping
  const EZYVET_TYPE_LABEL_MAP = [
    { ezyvet: ["intake", "initial consultation", "initial appointment", "initial assessment", "first appointment", "new client"], vetboard: ["intake", "new client", "evaluation"] },
    { ezyvet: ["diagnostic evaluation", "psychiatric evaluation", "psych eval", "assessment", "testing"], vetboard: ["evaluation", "assessment", "intake"] },
    { ezyvet: ["therapy session", "individual therapy", "psychotherapy", "individual session", "session", "53 min", "45 min", "60 min"], vetboard: ["session", "therapy"] },
    { ezyvet: ["couples", "couple therapy", "marital", "relationship"], vetboard: ["couples"] },
    { ezyvet: ["family therapy", "family session"], vetboard: ["family"] },
    { ezyvet: ["group therapy", "group session", "group"], vetboard: ["group"] },
    { ezyvet: ["telehealth", "video", "virtual", "video office", "phone consult", "phone consultation"], vetboard: ["telehealth", "virtual"] },
    { ezyvet: ["med management", "medication management", "psychiatry follow up", "med check"], vetboard: ["med management", "psychiatry"] },
    { ezyvet: ["crisis", "urgent", "emergency"], vetboard: ["crisis", "urgent", "emergency"] },
    { ezyvet: ["consult", "consultation", "15 minute", "30 minute"], vetboard: ["consult"] },
    { ezyvet: ["emdr", "dbt", "cbt", "play therapy"], vetboard: ["session", "therapy"] },
    { ezyvet: ["follow up", "follow-up", "check in", "check-in"], vetboard: ["follow-up", "session"] }
  ];
  ...
  // SimplePractice-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need SimplePractice's own
    ...
    // Wait briefly so SimplePractice's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the SimplePractice appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (SimplePractice's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by SimplePractice) never embed time text in the card body — the
    ...
    // ── SimplePractice card text pattern ────────────────────────────────────────────
    // SimplePractice cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── SimplePractice appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try SimplePractice-specific detail panel first (the right-side info pane)
    ...
    // Try to parse SimplePractice's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → SimplePractice card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → SimplePractice card parse → generic guess
    ...
    // Doctor: SimplePractice calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse SimplePractice card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip SimplePractice patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the SimplePractice right-side appointment detail panel
  // SimplePractice labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate SimplePractice's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // SimplePractice detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (SimplePractice format)
  ...
  // ── Color label matching (SimplePractice-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. SimplePractice type map — highest priority
  ...
  function findAliasColorLabel(colors, looseSearchText) {
    const aliasGroups = [
      { aliases: ["intake", "initial consultation", "initial appointment", "initial assessment", "first appointment", "new client"], labels: ["intake", "new client", "evaluation"] },
      { aliases: ["diagnostic evaluation", "psychiatric evaluation", "psych eval", "assessment", "testing"], labels: ["evaluation", "assessment", "intake"] },
      { aliases: ["therapy session", "individual therapy", "psychotherapy", "individual session", "session", "53 min", "45 min", "60 min"], labels: ["session", "therapy"] },
      { aliases: ["couples", "couple therapy", "marital", "relationship"], labels: ["couples"] },
      { aliases: ["family therapy", "family session"], labels: ["family"] },
      { aliases: ["group therapy", "group session", "group"], labels: ["group"] },
      { aliases: ["telehealth", "video", "virtual", "video office", "phone consult", "phone consultation"], labels: ["telehealth", "virtual"] },
      { aliases: ["med management", "medication management", "psychiatry follow up", "med check"], labels: ["med management", "psychiatry"] },
      { aliases: ["crisis", "urgent", "emergency"], labels: ["crisis", "urgent", "emergency"] },
      { aliases: ["consult", "consultation", "15 minute", "30 minute"], labels: ["consult"] },
      { aliases: ["emdr", "dbt", "cbt", "play therapy"], labels: ["session", "therapy"] },
      { aliases: ["follow up", "follow-up", "check in", "check-in"], labels: ["follow-up", "session"] }
    ];
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard SimplePractice capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the SimplePractice appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-simplepractice-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-simplepractice-styles";
    style.textContent = `
      .${HOVER_CLASS} {
