(function () {
  ...
  if (window.__vetBoardAdvancedmdCaptureLoaded) return;
  window.__vetBoardAdvancedmdCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-advancedmd-overlay";
  const TOAST_ID = "vetboard-advancedmd-toast";
  const BADGE_ID = "vetboard-advancedmd-badge";
  const AUTH_PANEL_ID = "vetboard-advancedmd-auth-panel";
  const MODAL_ID = "vetboard-advancedmd-modal";
  const BACKDROP_ID = "vetboard-advancedmd-backdrop";
  const HOVER_CLASS = "vetboard-advancedmd-hover";
  ...
  // AdvancedMD uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)advancedmd\.com$/i;
  ...
  // AdvancedMD card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // AdvancedMD appointment type → RoomBoard label mapping
  ...
  // AdvancedMD-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need AdvancedMD's own
    ...
    // Wait briefly so AdvancedMD's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the AdvancedMD appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (AdvancedMD's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by AdvancedMD) never embed time text in the card body — the
    ...
    // ── AdvancedMD card text pattern ────────────────────────────────────────────
    // AdvancedMD cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── AdvancedMD appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try AdvancedMD-specific detail panel first (the right-side info pane)
    ...
    // Try to parse AdvancedMD's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → AdvancedMD card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → AdvancedMD card parse → generic guess
    ...
    // Doctor: AdvancedMD calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse AdvancedMD card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip AdvancedMD patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the AdvancedMD right-side appointment detail panel
  // AdvancedMD labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate AdvancedMD's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // AdvancedMD detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (AdvancedMD format)
  ...
  // ── Color label matching (AdvancedMD-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. AdvancedMD type map — highest priority
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard AdvancedMD capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the AdvancedMD appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-advancedmd-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-advancedmd-styles";
    style.textContent = `
      .${HOVER_CLASS} {
