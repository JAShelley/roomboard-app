(function () {
  ...
  if (window.__vetBoardCurveDentalCaptureLoaded) return;
  window.__vetBoardCurveDentalCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-curve-dental-overlay";
  const TOAST_ID = "vetboard-curve-dental-toast";
  const BADGE_ID = "vetboard-curve-dental-badge";
  const AUTH_PANEL_ID = "vetboard-curve-dental-auth-panel";
  const MODAL_ID = "vetboard-curve-dental-modal";
  const BACKDROP_ID = "vetboard-curve-dental-backdrop";
  const HOVER_CLASS = "vetboard-curve-dental-hover";
  ...
  // Curve Dental uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)(?:curvehero\.com|curveapps\.com|curvedental\.com|curvegro\.com)$/i;
  ...
  // Curve Dental card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // Curve Dental appointment type → RoomBoard label mapping
  ...
  // Curve Dental-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need Curve Dental's own
    ...
    // Wait briefly so Curve Dental's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the Curve Dental appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (Curve Dental's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by Curve Dental) never embed time text in the card body — the
    ...
    // ── Curve Dental card text pattern ────────────────────────────────────────────
    // Curve Dental cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── Curve Dental appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try Curve Dental-specific detail panel first (the right-side info pane)
    ...
    // Try to parse Curve Dental's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → Curve Dental card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → Curve Dental card parse → generic guess
    ...
    // Doctor: Curve Dental calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse Curve Dental card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip Curve Dental patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the Curve Dental right-side appointment detail panel
  // Curve Dental labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate Curve Dental's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // Curve Dental detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (Curve Dental format)
  ...
  // ── Color label matching (Curve Dental-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. Curve Dental type map — highest priority
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard Curve Dental capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the Curve Dental appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-curve-dental-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-curve-dental-styles";
    style.textContent = `
      .${HOVER_CLASS} {
