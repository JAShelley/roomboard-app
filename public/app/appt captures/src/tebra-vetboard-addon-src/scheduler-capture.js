(function () {
  ...
  if (window.__vetBoardTebraCaptureLoaded) return;
  window.__vetBoardTebraCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-tebra-overlay";
  const TOAST_ID = "vetboard-tebra-toast";
  const BADGE_ID = "vetboard-tebra-badge";
  const AUTH_PANEL_ID = "vetboard-tebra-auth-panel";
  const MODAL_ID = "vetboard-tebra-modal";
  const BACKDROP_ID = "vetboard-tebra-backdrop";
  const HOVER_CLASS = "vetboard-tebra-hover";
  ...
  // Tebra/Kareo uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)(?:tebra\.com|kareo\.com)$/i;
  ...
  // Tebra/Kareo card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // Tebra/Kareo appointment type → RoomBoard label mapping
  ...
  // Tebra/Kareo-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need Tebra/Kareo's own
    ...
    // Wait briefly so Tebra/Kareo's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the Tebra/Kareo appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (Tebra/Kareo's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by Tebra/Kareo) never embed time text in the card body — the
    ...
    // ── Tebra/Kareo card text pattern ────────────────────────────────────────────
    // Tebra/Kareo cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── Tebra/Kareo appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try Tebra/Kareo-specific detail panel first (the right-side info pane)
    ...
    // Try to parse Tebra/Kareo's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → Tebra/Kareo card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → Tebra/Kareo card parse → generic guess
    ...
    // Doctor: Tebra/Kareo calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse Tebra/Kareo card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip Tebra/Kareo patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the Tebra/Kareo right-side appointment detail panel
  // Tebra/Kareo labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate Tebra/Kareo's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // Tebra/Kareo detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (Tebra/Kareo format)
  ...
  // ── Color label matching (Tebra/Kareo-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. Tebra/Kareo type map — highest priority
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard Tebra/Kareo capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the Tebra/Kareo appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-tebra-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-tebra-styles";
    style.textContent = `
      .${HOVER_CLASS} {
