(function () {
  ...
  if (window.__vetBoardDentrixAscendCaptureLoaded) return;
  window.__vetBoardDentrixAscendCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-dentrix-ascend-overlay";
  const TOAST_ID = "vetboard-dentrix-ascend-toast";
  const BADGE_ID = "vetboard-dentrix-ascend-badge";
  const AUTH_PANEL_ID = "vetboard-dentrix-ascend-auth-panel";
  const MODAL_ID = "vetboard-dentrix-ascend-modal";
  const BACKDROP_ID = "vetboard-dentrix-ascend-backdrop";
  const HOVER_CLASS = "vetboard-dentrix-ascend-hover";
  ...
  // Dentrix Ascend uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)(?:dentrixascend\.com|hsps\.pro|henryscheinone\.com)$/i;
  ...
  // Dentrix Ascend card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // Dentrix Ascend-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need Dentrix Ascend's own
    ...
    // Wait briefly so Dentrix Ascend's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the Dentrix Ascend appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (Dentrix Ascend's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by Dentrix Ascend) never embed time text in the card body — the
    ...
    // ── Dentrix Ascend card text pattern ────────────────────────────────────────────
    // Dentrix Ascend cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── Dentrix Ascend appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try Dentrix Ascend-specific detail panel first (the right-side info pane)
    ...
    // Try to parse Dentrix Ascend's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → Dentrix Ascend card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → Dentrix Ascend card parse → generic guess
    ...
    // Doctor: Dentrix Ascend calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse Dentrix Ascend card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip Dentrix Ascend patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the Dentrix Ascend right-side appointment detail panel
  // Dentrix Ascend labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate Dentrix Ascend's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // Dentrix Ascend detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (Dentrix Ascend format)
  ...
  // ── Color label matching (Dentrix Ascend-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. Dentrix Ascend type map — highest priority
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-dentrix-ascend-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-dentrix-ascend-styles";
    style.textContent = `
      .${HOVER_CLASS} {
