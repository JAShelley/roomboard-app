(function () {
  ...
  if (window.__vetBoardTab32CaptureLoaded) return;
  window.__vetBoardTab32CaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-tab32-overlay";
  const TOAST_ID = "vetboard-tab32-toast";
  const BADGE_ID = "vetboard-tab32-badge";
  const AUTH_PANEL_ID = "vetboard-tab32-auth-panel";
  const MODAL_ID = "vetboard-tab32-modal";
  const BACKDROP_ID = "vetboard-tab32-backdrop";
  const HOVER_CLASS = "vetboard-tab32-hover";
  ...
  // tab32 uses FullCalendar — target those event elements first, then fall back generically
  ...
  const HOST_HINT_RE = /(?:^|\.)tab32\.com$/i;
  ...
  // tab32 card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // tab32 appointment type → RoomBoard label mapping
  ...
  // tab32-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need tab32's own
    ...
    // Wait briefly so tab32's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the tab32 appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (tab32's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by tab32) never embed time text in the card body — the
    ...
    // ── tab32 card text pattern ────────────────────────────────────────────
    // tab32 cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  function parseAppointment(card) {
    ...
    // Try tab32-specific detail panel first (the right-side info pane)
    ...
    // Try to parse tab32's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → tab32 card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → tab32 card parse → generic guess
    ...
    // Doctor: tab32 calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse tab32 card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip tab32 patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the tab32 right-side appointment detail panel
  // tab32 labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate tab32's right-side detail panel (appears on click/hover)
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (tab32 format)
  ...
  // ── Color label matching (tab32-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. tab32 type map — highest priority
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard tab32 capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the tab32 appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-tab32-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-tab32-styles";
    style.textContent = `
      .${HOVER_CLASS} {
