(function () {
  ...
  if (window.__vetBoardDenticonCaptureLoaded) return;
  window.__vetBoardDenticonCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-denticon-overlay";
  const TOAST_ID = "vetboard-denticon-toast";
  const BADGE_ID = "vetboard-denticon-badge";
  const AUTH_PANEL_ID = "vetboard-denticon-auth-panel";
  const MODAL_ID = "vetboard-denticon-modal";
  const BACKDROP_ID = "vetboard-denticon-backdrop";
  const HOVER_CLASS = "vetboard-denticon-hover";
  ...
  // Denticon's appointment book URLs (ASPX/Appointments/appttrial.aspx) point to a legacy
  // ASP.NET WebForms grid, not a FullCalendar-style JS calendar — the FullCalendar selectors
  // below are an unverified guess kept for compatibility if a newer Angular surface is in use.
  // The WebForms/Telerik-grid selectors are a lower-confidence fallback tier added alongside
  // them; neither set has been confirmed against a live Denticon account.
  const CARD_SELECTORS = [
    ...
    // Legacy ASP.NET WebForms / Telerik RadGrid fallback (unverified — see comment above)
    ".rgMasterTable tr[role='row']",
    "table[id*='Grid'] tr",
    "[id*='Appointment'][id*='ctl0']",
    "[id*='Appt'] tr",
    "[class*='apptBlock']",
    "[class*='ApptBlock']"
  ];
  ...
  const DOCTOR_RE = /\b(?:dr\.?|doctor|dds|dmd|hygienist|rdh|assistant|tech)\b/i;
  const DOCTOR_NAME_RE = /\b(?:dr\.?|doctor|dds|d\.d\.s\.|dmd|d\.m\.d\.|rdh)\b/i;
  // Fast-path page detection for this platform's real scheduler domains.
  const HOST_HINT_RE = /(?:^|\.)(?:denticon\.com|planetdds\.com)$/i;
  ...
  // Denticon card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // Denticon appointment type → RoomBoard label mapping
  const EZYVET_TYPE_LABEL_MAP = [
    ...
    { ezyvet: ["limited exam", "limited eval", "problem focused", "emergency exam"], vetboard: ["limited exam", "exam"] },
    { ezyvet: ["prophy", "prophylaxis", "cleaning", "hygiene", "recall", "recare", "perio maintenance", "pm"], vetboard: ["hygiene", "prophy", "cleaning", "recall"] },
    { ezyvet: ["srp", "scaling and root planing", "root planing", "deep cleaning", "perio therapy", "periodontal"], vetboard: ["perio", "srp", "deep cleaning"] },
    { ezyvet: ["crown", "bridge", "onlay", "inlay", "crown prep", "crown seat", "crown delivery", "cerec"], vetboard: ["crown", "restorative"] },
    { ezyvet: ["filling", "composite", "restorative", "operative", "resin", "amalgam"], vetboard: ["filling", "restorative"] },
    { ezyvet: ["extraction", "ext", "exts", "oral surgery", "surgical", "wisdom"], vetboard: ["extraction", "oral surgery", "surgery"] },
    { ezyvet: ["root canal", "endo", "endodontic", "rct", "pulpotomy"], vetboard: ["root canal", "endo"] },
    { ezyvet: ["implant", "implant placement", "implant restoration"], vetboard: ["implant", "surgery"] },
    { ezyvet: ["denture", "partial", "prosth", "reline", "delivery"], vetboard: ["denture", "prosth"] },
    { ezyvet: ["ortho", "orthodontic", "invisalign", "aligner", "braces", "adjustment"], vetboard: ["ortho"] },
    { ezyvet: ["whitening", "cosmetic", "veneer", "bleaching"], vetboard: ["cosmetic", "whitening"] },
    { ezyvet: ["consult", "consultation", "treatment plan", "tx plan"], vetboard: ["consult"] },
    { ezyvet: ["post op", "post-op", "follow up", "follow-up", "recheck", "suture removal", "adjustment check"], vetboard: ["follow-up", "post op"] },
    { ezyvet: ["x-ray", "xray", "fmx", "pano", "panoramic", "bitewing", "imaging", "radiograph"], vetboard: ["imaging", "x-ray"] }
  ];
  ...
  // Denticon-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need Denticon's own
    ...
    // Wait briefly so Denticon's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the Denticon appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (Denticon's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by Denticon) never embed time text in the card body — the
    ...
    // ── Denticon card text pattern ────────────────────────────────────────────
    // Denticon cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  function parseAppointment(card) {
    ...
    // Try Denticon-specific detail panel first (the right-side info pane)
    ...
    // Try to parse Denticon's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → Denticon card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → Denticon card parse → generic guess
    ...
    // Doctor: Denticon calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse Denticon card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip Denticon patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the Denticon right-side appointment detail panel
  // Denticon labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate Denticon's right-side detail panel (appears on click/hover)
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (Denticon format)
  ...
  // ── Color label matching (Denticon-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. Denticon type map — highest priority
  ...
  function findAliasColorLabel(colors, looseSearchText) {
    const aliasGroups = [
      { aliases: ["emergency", "toothache", "tooth pain", "dental pain", "swelling", "abscess", "broken tooth", "urgent"], labels: ["emergency", "urgent"] },
      { aliases: ["new patient", "np exam", "comprehensive exam", "comp exam", "new pt"], labels: ["new patient", "comprehensive exam", "exam"] },
      { aliases: ["limited exam", "limited eval", "problem focused", "emergency exam"], labels: ["limited exam", "exam"] },
      { aliases: ["prophy", "prophylaxis", "cleaning", "hygiene", "recall", "recare", "perio maintenance", "pm"], labels: ["hygiene", "prophy", "cleaning", "recall"] },
      { aliases: ["srp", "scaling and root planing", "root planing", "deep cleaning", "perio therapy", "periodontal"], labels: ["perio", "srp", "deep cleaning"] },
      { aliases: ["crown", "bridge", "onlay", "inlay", "crown prep", "crown seat", "crown delivery", "cerec"], labels: ["crown", "restorative"] },
      { aliases: ["filling", "composite", "restorative", "operative", "resin", "amalgam"], labels: ["filling", "restorative"] },
      { aliases: ["extraction", "ext", "exts", "oral surgery", "surgical", "wisdom"], labels: ["extraction", "oral surgery", "surgery"] },
      { aliases: ["root canal", "endo", "endodontic", "rct", "pulpotomy"], labels: ["root canal", "endo"] },
      { aliases: ["implant", "implant placement", "implant restoration"], labels: ["implant", "surgery"] },
      { aliases: ["denture", "partial", "prosth", "reline", "delivery"], labels: ["denture", "prosth"] },
      { aliases: ["ortho", "orthodontic", "invisalign", "aligner", "braces", "adjustment"], labels: ["ortho"] },
      { aliases: ["whitening", "cosmetic", "veneer", "bleaching"], labels: ["cosmetic", "whitening"] },
      { aliases: ["consult", "consultation", "treatment plan", "tx plan"], labels: ["consult"] },
      { aliases: ["post op", "post-op", "follow up", "follow-up", "recheck", "suture removal", "adjustment check"], labels: ["follow-up", "post op"] },
      { aliases: ["x-ray", "xray", "fmx", "pano", "panoramic", "bitewing", "imaging", "radiograph"], labels: ["imaging", "x-ray"] }
    ];
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard Denticon capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the Denticon appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-denticon-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-denticon-styles";
    style.textContent = `
      .${HOVER_CLASS} {
