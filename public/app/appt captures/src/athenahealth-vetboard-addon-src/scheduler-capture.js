(function () {
  ...
  if (window.__vetBoardAthenahealthCaptureLoaded) return;
  window.__vetBoardAthenahealthCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-athenahealth-overlay";
  const TOAST_ID = "vetboard-athenahealth-toast";
  const BADGE_ID = "vetboard-athenahealth-badge";
  const AUTH_PANEL_ID = "vetboard-athenahealth-auth-panel";
  const MODAL_ID = "vetboard-athenahealth-modal";
  const BACKDROP_ID = "vetboard-athenahealth-backdrop";
  const HOVER_CLASS = "vetboard-athenahealth-hover";
  ...
  // athenahealth uses FullCalendar — target those event elements first, then fall back generically
  ...
  const DOCTOR_RE = /\b(?:dr\.?|doctor|md|d\.o\.|do|np|pa-?c?|aprn|dnp|nurse|ma)\b/i;
  const DOCTOR_NAME_RE = /\b(?:dr\.?|doctor|m\.?d\.?|d\.?o\.?|np|pa-?c|aprn|dnp)\b/i;
  // Fast-path page detection for this platform's real scheduler domains.
  const HOST_HINT_RE = /(?:^|\.)(?:athenahealth\.com|athena\.io)$/i;
  ...
  // athenahealth card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  // athenahealth appointment type → RoomBoard label mapping
  const EZYVET_TYPE_LABEL_MAP = [
    { ezyvet: ["annual", "physical", "wellness", "awv", "annual wellness visit", "preventive", "well visit"], vetboard: ["annual", "physical", "wellness"] },
    { ezyvet: ["well child", "well-child", "well baby", "pediatric wellness"], vetboard: ["well child", "wellness"] },
    { ezyvet: ["new patient", "np visit", "new pt", "establish care", "establishing care"], vetboard: ["new patient"] },
    { ezyvet: ["follow up", "follow-up", "followup", "f/u", "established", "return visit", "recheck"], vetboard: ["follow-up"] },
    { ezyvet: ["sick", "sick visit", "acute", "illness", "problem visit", "same day", "same-day"], vetboard: ["sick visit", "acute"] },
    { ezyvet: ["urgent", "urgent care", "work in", "work-in", "walk in", "walk-in"], vetboard: ["urgent", "work-in"] },
    { ezyvet: ["telehealth", "telemedicine", "virtual", "video visit", "phone visit"], vetboard: ["telehealth", "virtual"] },
    { ezyvet: ["procedure", "minor procedure", "biopsy", "injection", "joint injection", "mole removal"], vetboard: ["procedure"] },
    { ezyvet: ["labs", "lab draw", "blood draw", "lab work", "phlebotomy", "lab only"], vetboard: ["labs", "blood draw"] },
    { ezyvet: ["vaccine", "vaccination", "immunization", "flu shot", "shot only"], vetboard: ["vaccine", "immunization"] },
    { ezyvet: ["med check", "medication management", "med refill", "medication review"], vetboard: ["med check", "medication"] },
    { ezyvet: ["consult", "consultation", "referral", "specialist"], vetboard: ["consult"] },
    { ezyvet: ["pre op", "pre-op", "preoperative", "surgical clearance", "pre-surgical"], vetboard: ["pre-op", "clearance"] },
    { ezyvet: ["hospital follow up", "er follow up", "tcm", "transitional care"], vetboard: ["follow-up"] },
    { ezyvet: ["physical therapy", "pt visit"], vetboard: ["therapy"] },
    { ezyvet: ["emergency"], vetboard: ["emergency"] }
  ];
  ...
  // athenahealth-specific page detection: fast-path on hostname, then fall back to heuristics
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need athenahealth's own
    ...
    // Wait briefly so athenahealth's handlers run and open the detail panel, then
  ...
  async function captureVisibleEzyvetSummary(event) {
    ...
    if (!summary?.patientName) {
      showToast("Hover an appointment until the athenahealth appointment details appears, then click.");
      refreshEzyvetHoverSelection();
      return;
    }
  ...
  // Returns true for any FullCalendar event node (athenahealth's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by athenahealth) never embed time text in the card body — the
    ...
    // ── athenahealth card text pattern ────────────────────────────────────────────
    // athenahealth cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // ── athenahealth appointment parsing ──────────────────────────────────────────────
  ...
  function parseAppointment(card) {
    ...
    // Try athenahealth-specific detail panel first (the right-side info pane)
    ...
    // Try to parse athenahealth's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → athenahealth card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → athenahealth card parse → generic guess
    ...
    // Doctor: athenahealth calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse athenahealth card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Strip athenahealth patient name artifacts like "(527357)" or breed suffixes
  ...
  // Parse the athenahealth right-side appointment detail panel
  // athenahealth labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate athenahealth's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      // athenahealth detail panel signals
      const hasCaseOwner = /case owner/i.test(text);
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (athenahealth format)
  ...
  // ── Color label matching (athenahealth-aware) ─────────────────────────────────────
  ...
  function findBestColorLabelId(boardData, appointment) {
    ...
    // 1. athenahealth type map — highest priority
  ...
  function findAliasColorLabel(colors, looseSearchText) {
    const aliasGroups = [
      { aliases: ["annual", "physical", "wellness", "awv", "annual wellness visit", "preventive", "well visit"], labels: ["annual", "physical", "wellness"] },
      { aliases: ["well child", "well-child", "well baby", "pediatric wellness"], labels: ["well child", "wellness"] },
      { aliases: ["new patient", "np visit", "new pt", "establish care", "establishing care"], labels: ["new patient"] },
      { aliases: ["follow up", "follow-up", "followup", "f/u", "established", "return visit", "recheck"], labels: ["follow-up"] },
      { aliases: ["sick", "sick visit", "acute", "illness", "problem visit", "same day", "same-day"], labels: ["sick visit", "acute"] },
      { aliases: ["urgent", "urgent care", "work in", "work-in", "walk in", "walk-in"], labels: ["urgent", "work-in"] },
      { aliases: ["telehealth", "telemedicine", "virtual", "video visit", "phone visit"], labels: ["telehealth", "virtual"] },
      { aliases: ["procedure", "minor procedure", "biopsy", "injection", "joint injection", "mole removal"], labels: ["procedure"] },
      { aliases: ["labs", "lab draw", "blood draw", "lab work", "phlebotomy", "lab only"], labels: ["labs", "blood draw"] },
      { aliases: ["vaccine", "vaccination", "immunization", "flu shot", "shot only"], labels: ["vaccine", "immunization"] },
      { aliases: ["med check", "medication management", "med refill", "medication review"], labels: ["med check", "medication"] },
      { aliases: ["consult", "consultation", "referral", "specialist"], labels: ["consult"] },
      { aliases: ["pre op", "pre-op", "preoperative", "surgical clearance", "pre-surgical"], labels: ["pre-op", "clearance"] },
      { aliases: ["hospital follow up", "er follow up", "tcm", "transitional care"], labels: ["follow-up"] },
      { aliases: ["physical therapy", "pt visit"], labels: ["therapy"] },
      { aliases: ["emergency"], labels: ["emergency"] }
    ];
  ...
  function updateBadgeUi() {
    ...
    let label = "RoomBoard athenahealth capture idle. Click to arm. Right-click to log in.";
    if (authNeedsLogin) {
      label = authErrorMessage || "RoomBoard needs you to sign in again. Right-click for login.";
    } else if (captureArmed) {
      label = isEzyvetHost()
        ? "Armed — hover an appointment until the athenahealth appointment details appears, then click."
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-athenahealth-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-athenahealth-styles";
    style.textContent = `
      .${HOVER_CLASS} {
