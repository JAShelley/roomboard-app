(function () {
  ...
  if (window.__vetBoardEzyvetCaptureLoaded) return;
  window.__vetBoardEzyvetCaptureLoaded = true;
  ...
  const OVERLAY_ID = "vetboard-ezyvet-overlay";
  const TOAST_ID = "vetboard-ezyvet-toast";
  const BADGE_ID = "vetboard-ezyvet-badge";
  const AUTH_PANEL_ID = "vetboard-ezyvet-auth-panel";
  const MODAL_ID = "vetboard-ezyvet-modal";
  const BACKDROP_ID = "vetboard-ezyvet-backdrop";
  const HOVER_CLASS = "vetboard-ezyvet-hover";
  ...
  // ezyVet uses FullCalendar — target those event elements first, then fall back generically
  ...
  // ezyVet card text format: "*PatientName" OwnerLast (Species (Breed)) - CaseNum - Reason
  ...
  const EZYVET_TYPE_LABEL_MAP = [
    ...
    { ezyvet: ["sx consult", "surgery consult", "surgical referral", "surgical consult", "specialist consult", "orthopedic", "ortho", "spay neuter consult"], vetboard: ["sx consult", "surgery consult"] },
    { ezyvet: ["surgery", "surgical", "spay", "neuter", "dental surgery", "procedure", "ovariohysterectomy"], vetboard: ["sx consult", "surgery"] },
    { ezyvet: ["wellness", "annual exam", "annual wellness", "new puppy", "new kitten", "new patient exam", "puppy kitten"], vetboard: ["exam", "wellness"] },
    { ezyvet: ["ecc- recheck", "ecc recheck", "recheck", "follow up", "follow-up", "followup", "post op", "post-op"], vetboard: ["exam"] },
    { ezyvet: ["exam", "general exam", "physical exam", "comprehensive exam"], vetboard: ["exam"] },
    { ezyvet: ["tech", "tech visit", "tech appointment", "tech appt", "walk back", "nurse visit", "ecc- tech"], vetboard: ["tech/walkback", "tech appt", "tech"] },
    { ezyvet: ["illness", "injury", "illness/injury", "sick visit", "injury visit", "problem visit"], vetboard: ["illness/injury"] },
    { ezyvet: ["ultrasound", "ultra sound", "u/s", "abd u/s", "abdominal ultrasound", "echocardiogram", "echo"], vetboard: ["ultrasound", "u/s"] },
    { ezyvet: ["drop off", "drop-off", "day patient", "day admit", "sedation drop off", "boarding admit"], vetboard: ["drop-off", "drop off"] },
    { ezyvet: ["outside contagious", "contagious", "isolation", "car visit", "curbside"], vetboard: ["outside**contagious", "car - isolation"] },
    { ezyvet: ["work-in", "work in", "urgent care", "urgent", "same day"], vetboard: ["work-in", "work in"] },
    { ezyvet: ["ecc- scheduled procedure", "scheduled procedure", "procedure"], vetboard: ["sx consult"] }
  ];
  ...
  // ezyVet-specific page detection: fast-path on hostname, then fall back to heuristics
  function looksLikeEzyvetPage() {
    if (isEzyvetHost()) return true;
  ...
  async function onDocumentClick(event) {
    ...
    // call stopPropagation/stopImmediatePropagation here — we need ezyVet's own
    ...
    // Wait briefly so ezyVet's handlers run and open the detail panel, then
  ...
  // Returns true for any FullCalendar event node (ezyVet's calendar engine)
  ...
  function isLikelyAppointmentCard(node) {
    ...
    // ── FullCalendar events ─────────────────────────────────────────────────
    // FC events (used by ezyVet) never embed time text in the card body — the
    ...
    // ── ezyVet card text pattern ────────────────────────────────────────────
    // ezyVet cards: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  function parseAppointment(card) {
    ...
    // Try ezyVet-specific detail panel first (the right-side info pane)
    ...
    // Try to parse ezyVet's card text format first
    const cardLineText = cleaned.join(" ");
    ...
    // Patient name: prefer detail panel → ezyVet card parse → generic guess
    const patientName = normalizeSpaces(
    ...
    // Reason: prefer detail panel → ezyVet card parse → generic guess
    ...
    // Doctor: ezyVet calls this "case owner" in the detail panel
    const doctor = normalizeSpaces(
      hoverDetails.provider ||
  ...
  // Parse ezyVet card text: "*PatientName" OwnerLast (Species) - CaseNum - Reason
  ...
  // Parse the ezyVet right-side appointment detail panel
  // ezyVet labels: PATIENT, OWNER, TYPE, DATE, TIME, CASE OWNER, STATUS, PRESENTING PROBLEM(S)
  ...
  // Locate ezyVet's right-side detail panel (appears on click/hover)
  function findEzyvetDetailPanel(card) {
    ...
    for (const node of candidates) {
      ...
      const signalCount = [hasCaseOwner, hasType, hasPatient, hasReason, hasPresenting, hasOwner, hasStatus, hasVisitHighlights].filter(Boolean).length;
      ...
      const score = (looksLikeTooltip ? 420 : 0) + (hasCaseOwner ? 300 : 0) + (hasReason ? 280 : 0) +
        ...
        (hasType ? 120 : 0) + (hasPatient ? 120 : 0) + (hasOwner ? 90 : 0) + (hasStatus ? 80 : 0) +
        (overlapsCardY ? 160 : 0) -
  ...
  function guessPatientName(lines) {
    for (const line of lines) {
      ...
      // Try to extract quoted name (ezyVet format)
  ...
  // ── Color label matching (ezyVet-aware) ─────────────────────────────────────
  ...
  function findAliasColorLabel(colors, looseSearchText) {
    const aliasGroups = [
      { aliases: ["euth", "euthanasia", "pts", "quality of life", "qol"], labels: ["euthanasia consult", "euthanasia"] },
      { aliases: ["sx", "surgery", "surgical", "spay", "neuter", "dental", "consult"], labels: ["sx consult", "surgery consult", "surgery"] },
      { aliases: ["drop off", "dropoff", "day admit", "admit", "day patient"], labels: ["drop-off", "drop off"] },
      { aliases: ["tech", "walk back", "walkback", "nurse"], labels: ["tech appt", "tech"] },
      { aliases: ["recheck", "follow up", "followup", "post op", "new puppy", "new kitten", "new patient", "exam", "annual", "wellness"], labels: ["exam", "wellness"] },
      { aliases: ["vaccine", "vacc", "booster", "rabies", "dhpp", "bordetella", "lepto"], labels: ["vaccine", "vacc", "wellness"] },
      { aliases: ["illness injury", "illness/injury", "injury illness", "illness", "injury", "sick", "urgent"], labels: ["illness/injury", "illness injury"] },
      { aliases: ["outside contagious", "contagious", "car", "isolation", "curbside"], labels: ["outside contagious", "car isolation", "car - isolation"] },
      { aliases: ["ultrasound", "u/s", "echo", "echocardiogram"], labels: ["ultrasound", "u/s"] },
      { aliases: ["emergency", "ecc", "critical care"], labels: ["emergency"] }
    ];
  ...
  function injectStyles() {
    if (document.getElementById("vetboard-ezyvet-styles")) return;
    const style = document.createElement("style");
    style.id = "vetboard-ezyvet-styles";
    style.textContent = `
      .${HOVER_CLASS} {
